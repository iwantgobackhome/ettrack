...
class TransconverTracker(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # info에 init_bbox 있는 경우
        if 'init_bbox' in info:
            if type(info['init_bbox']) == collections.OrderedDict:
                # web_cam 일 경우 collections.OrderedDict 타입이므로 ettrack 은 SOT이기 때문에 init_box하나로 고정
                try:
                    state = torch.tensor(info['init_bbox'][1])
                except:
                    state = info['init_bbox'] # 초기화된 경우
            else:
                state = torch.tensor(info['init_bbox']) # x,y,w,h
            # reset 하는 경우 등 init_bbox가 비어있는 경우
            if len(info['init_bbox']) > 0:
            ...
            # 초반 init 후에 다시 호출 되어서 init_bbox 없는 경우
            
            # 초반 init 후에 추적의 경우 im_w, im_h 값 필요
            state_dict['im_w'] = info['previous_output']['im_w']
            state_dict['im_h'] = info['previous_output']['im_h']
            state_dict['p'] = info['previous_output']['p']
            state_dict['avg_chans'] = info['previous_output']['avg_chans']
            state_dict['window'] = info['previous_output']['window']
            state_dict['target_pos'] = self.target_pos
            state_dict['target_sz'] = self.target_sz
            state_dict['time'] = time.time() - tic
        return state_dict
    ...
    def track(self, im, state_input, writer=None):
        if 'p' not in state_input['previous_output']:
            state = self.initialize(im, state_input)
        else:
            self.frame_num += 1
